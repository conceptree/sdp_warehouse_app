...
public class DeliveryDaoImp implements DeliveryDao<Delivery> {
    ...
    @Override
    public void update(Delivery delivery, int delivery_id) {
        String sql = "UPDATE deliveries SET item_id=?, address=?,  quantity=? WHERE delivery_id=?";
        int insert = jdbcTemplate.update(sql, delivery.getItem_id(), delivery.getAddress(), delivery.getQuantity(), delivery_id);
        if(insert == 1){
            System.out.printf("Delivery Successfully updated!");
        }
    }
