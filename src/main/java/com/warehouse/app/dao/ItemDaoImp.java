...
public class ItemDaoImp implements ItemDao<Item> {
    ...
    public void update(Item item, int item_id) {
        String sql = "UPDATE items SET typeName=?, description=? WHERE item_id=?";
        int insert = jdbcTemplate.update(sql, item.getTypeName(), item.getDescription(), item_id);
        if(insert == 1){
            System.out.printf("Item Successfully updated!");
        }
    }
